...
function NavBar() {
  ...
  // Current language display (you can map more nicely if needed)
  const currentLang = i18n.language.toUpperCase(); // 'EN', 'FR', 'AR'

  const changeLanguage = (lng) => {
    i18n.changeLanguage(lng.toLowerCase());
    // Optional: close dropdowns/menus after language change
  ...
  const servicesData = [
    ...
    },
    {
      ...
      image: Assets.Images.Common.CreativeDesignLG, // ← you had same image here originally
      ...
      <div className="fixed top-0 left-0 right-0 z-[60] transition-transform duration-300 px-6 md:px-10 py-4 flex items-center justify-between bg-white shadow-sm">
            ...
            <div
              ...
              onClick={(e) => {
                ...
                if (item.id === "services") {
                  ...
                  // Desktop industry dropdown toggle (you had it separate)
                  // Here we reuse activeDropdown for simplicity
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <div className="relative">
            <button
              className="flex items-center gap-1 font-medium text-black hover:text-blue-600"
              onClick={() => {
                // You can toggle a dropdown here if you want more languages visible
                // For simplicity we can just cycle or show options
            ...
            {/* Simple language dropdown – you can style/improve it */}
            <div className="absolute top-10 right-0 bg-black text-white rounded-lg py-2 w-32 shadow-xl z-50 hidden group-hover:block">
              <button
                className="block w-full text-left px-4 py-2 hover:bg-blue-600 text-sm"
        ...
        <div className="pt-32 px-8 pb-10">
          ...
          <div className="mt-12 space-y-8">
            <div className="flex flex-wrap gap-5 text-white font-medium">
              <button
                className={`px-3 py-2 rounded ${currentLang === "EN" ? "bg-blue-600" : "opacity-60"}`}
                onClick={() => changeLanguage("en")}
              >
                EN
              </button>
              <button
                className={`px-3 py-2 rounded ${currentLang === "FR" ? "bg-blue-600" : "opacity-60"}`}
                onClick={() => changeLanguage("fr")}
              >
                FR
              </button>
              <button
                className={`px-3 py-2 rounded ${currentLang === "AR" ? "bg-blue-600" : "opacity-60"}`}
                onClick={() => changeLanguage("ar")}
              >
                AR
              </button>
